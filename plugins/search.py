...
async def search(bot, message):
    ...
    try:
        ...
        if not results:
            # Fetch IMDb results if no results found
            movies = await search_imdb(query)
            buttons = []
            for movie in movies: 
                buttons.append([InlineKeyboardButton(movie['title'], callback_data=f"recheck_{movie['id']}")])
            
            # Send message with buttons (no image preview)
            msg = await message.reply_text(
                text="<b><I>🔻 I Couldn't find anything related to Your Query😕.\n🔺 Did you mean any of these?</I></b>", 
...
async def recheck(bot, update):
    ...
    try:
        ...
        # Send the search results back to the user who requested the action
        await update.message.reply_text(
            text=f"🔍 Here are the results for your request:\n\n{head + results}",
...
async def request(bot, update):
    ...
    # Remove the previous message from the user (optional)
